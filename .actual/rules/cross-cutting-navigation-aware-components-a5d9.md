# Standardize React Navigation with TypeScript Type-Safe Route Contracts: Navigation Aware Components
...
- **R-NAV-001** MUST: Navigation-aware components MUST use @react-navigation/stack or @react-navigation/native navigation primitives.
...
- **R-NAV-005** MUST: Ensure TypeScript compilation succeeds without navigation-related type errors (tsc --noEmit).
- **R-NAV-006** SHOULD: Integrate navigation type checking into CI pipeline using tsc --noEmit to catch type errors before merge.
- **R-NAV-007** SHOULD: Document navigation structure and parameter contracts in component JSDoc comments or separate architecture documentation.
- **R-NAV-008** SHOULD: Limit navigation stack depth and use composition patterns to avoid deeply nested type definitions.
