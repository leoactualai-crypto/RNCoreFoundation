...
1. MUST: Authentication service boundaries MUST be accessed through the BASE_URL configuration variable with standardized endpoint paths (/auth/local, /auth/local/register)

## Policy Block

- MUST Authentication service boundaries MUST be accessed through the BASE_URL configuration variable with standardized endpoint paths (/auth/local, /auth/local/register)
