# Adopt StyleSheet.create() for Component Styling in React Native: Style Definitions Located
...
- **R-STYLE-001** MUST: Style definitions MUST be co-located with their component definitions in the same file using StyleSheet.create().
- **R-STYLE-002** SHOULD: Style definitions SHOULD be defined at the module level, typically at the bottom of the file after component definitions.
- **R-STYLE-003** SHOULD: Dynamic styles SHOULD use array syntax to combine static StyleSheet styles with computed style objects: style={[styles.base, dynamicStyle]}.
- **R-STYLE-004** SHOULD: Style keys SHOULD be descriptive and reflect component structure (e.g., container, title, button, text, icon).
- **R-STYLE-005** MAY: Inline style objects MAY be used only for highly dynamic styles computed at runtime based on props, state, or theme values that cannot be statically defined.
