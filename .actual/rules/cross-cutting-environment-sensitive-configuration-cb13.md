# Adopt Redux Toolkit with Thunk Middleware for State Management with Environment-Aware Configuration: Environment Sensitive Configuration

These rules are ALWAYS ACTIVE for all Redux store configuration files and state management code within the application.

### Rules

- **R-REDUX-001** SHOULD: Environment-sensitive configuration SHOULD be isolated to store initialization rather than distributed across reducer logic.
...
# Verify process.env.NODE_ENV is accessed only in store initialization
...
<enforcement>
Claude Code MUST NOT skip or defer verification. Static analysis scanning for @reduxjs/toolkit and redux-thunk imports in store configuration files is mandatory. TypeScript compilation enforcing AppDispatch and AppThunk type contract usage is mandatory. Code review checklist verifying rootReducer composition and environment variable access patterns is mandatory. Automated tests validating store behavior across different NODE_ENV values are mandatory.
</enforcement>