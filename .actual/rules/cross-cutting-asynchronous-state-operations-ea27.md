# Adopt Redux Toolkit with Thunk Middleware for State Management with Environment-Aware Configuration: Asynchronous State Operations

These rules are ALWAYS ACTIVE for all state management code in the application, particularly files in `template/src/store/` and any modules that dispatch asynchronous actions or configure the Redux store.

### Rules

- **R-REDUX-001** MUST: Asynchronous state operations MUST use redux-thunk middleware for side effect handling.
