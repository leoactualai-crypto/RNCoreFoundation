...
1. SHOULD: Event handlers and callback functions that depend on component state SHOULD be wrapped with useCallback to prevent unnecessary re-renders.

## Policy Block

- SHOULD Event handlers and callback functions that depend on component state SHOULD be wrapped with useCallback to prevent unnecessary re-renders.
