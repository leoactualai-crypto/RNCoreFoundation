...
1. MUST: Authentication service boundaries MUST be defined using axios HTTP client with explicit POST requests to /auth/local and /auth/local/register endpoints

## Policy Block

- MUST Authentication service boundaries MUST be defined using axios HTTP client with explicit POST requests to /auth/local and /auth/local/register endpoints
