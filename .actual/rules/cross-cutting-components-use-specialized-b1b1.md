# Standardize React Hooks for UI State Management in Public API Components: Components Use Specialized

These rules are ALWAYS ACTIVE for all screen components, navigators, and custom hooks exported as public API contracts in React Native applications, including LoginScreen, ChatScreen, AuthStackNavigator, and useAuth implementations.

### Rules

- **R-HOOKS-001** MAY: Components MAY use specialized hooks (useCallback) for callback memoization when passing functions to child components.
...
- All hook dependency arrays are correctly specified and verified by exhaustive-deps rule
